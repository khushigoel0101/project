...
const Navbar = () => {
  ...
  return (
    ...
    <nav className="container mx-auto flex items-center justify-between py-4 px-6">
      {/* Logo */}
      <div>
        <Link to="/" className="text-xl font-limelight font-medium ">
          Xyz-store
        </Link>
      </div>
      {/* Navigation Links */}
      ...
      <div className="flex items-center space-x-4">
        {user && user.role === "admin" && (<Link to="/admin" className='black bg-black px-2 rounded text-sm text-white'>Admin</Link>)}
        ...
        {cartItemCount > 0 && (<span className="absolute -top-1 bg-black text-white text-xs rounded-full px-2 py-0.5" >
        {cartItemCount}
        </span>)}
    ...
    {/* mobile nav */}
    <div className={`fixed top-0 left-0 w-3/4 sm:w-1/3 md:w-1/3 h-full bg-white shadow-lg transform 
